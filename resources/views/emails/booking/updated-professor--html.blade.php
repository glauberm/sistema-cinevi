...
@section('content')
    ...
    @component('components.paragraph')
        ...
        <strong>{{ Carbon\CarbonImmutable::parse($booking->devolution_date)->format('d/m/Y') }}</strong> 
        e tem você como professor responsável acaba de ser registrada por <strong>{{ $booking->owner->name }}</strong>.
    @endcomponent
